function syncCommuteFinal(filteredEvents = []) {
  ...
  filteredEvents.forEach(event => {
    ...
    let originLocation = homeAddress, finalArrivalBuffer = arrivalBuffer, finalPrepBuffer = prepBuffer;
    // Regex to look for "Start: " or "Origin: " followed by the location text
    // The 'i' makes it case-insensitive so "start:" or "START:" both work
    const customOriginMatch = desc?.match(/(?:Start|Origin):\s*(.+)/i);
    ...
    // If custom Origin is found, override the default originLocation
    if (customOriginMatch?.[1]?.trim()) {
      originLocation = customOriginMatch?.[1]?.trim() || homeAddress;
      // console.log("📍 Custom origin found for event '" + title);
    }
